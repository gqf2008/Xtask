//! CH583 中断胶水:SysTick 二合一处理(真 tick / SWIE 软中断)与切换入口。
//! 与 ch32v103/port.rs 同构——CH583 的 `SWIE` 也在 `CTLR`(bit31),可直接复用语义。
...
///   (`scheduler::systick`),返回 true 则请求切换;
...
unsafe extern "C" fn switch_context() {
    Ch583Porting::disable_irq(); // 防 SWIE 残留立即重入
    scheduler::schedule();
}
