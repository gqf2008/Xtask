...
- **`src/allocator.rs`** — global heap allocator (`linked_list_allocator`); `pub mod allocator` re-exports the engine for examples — the `tlsf` feature swaps `XTaskAllocer`'s inner engine to the hand-written mini TLSF (`allocator/tlsf.rs`, ch27; `FirstFit` is the same-shape first-fit wrapper for A/B experiments); apps call `xtask::init_heap(start_addr, size)` before spawning. `src/logger.rs` — `log` facade over RTT or stdout.
