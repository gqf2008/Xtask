...
- RISCV
  ...
  - [x] ESP32C3: esp32c3(PAC 直依赖;**真身是 RV32IMC,按 `riscv32imc` 构建**(2026-09-19 起进门禁)——同示例用 `riscv32imac` 编出的产物实测含 12 条 A 扩展指令,真芯片会非法指令,改用真 ISA 后 0 条;真机待验;**刷写配方已做格式级验证**(2026-09-19,esptool 5.4.0):`esptool --chip esp32c3 elf2image 直接吃下本口 imc 产物,image_info 复核为 magic 0xE9 / 3 段 / entry 0x42000000 / Chip ID 5;走 IDF bootloader 路线还需补 app 描述符 .flash.appdesc,否则走 direct boot 直接烧 0x0)
  ...
  - [x] CH58x: ch583(WCH CH583,QingKe V4A;构建级验证 2026-09-19,真机待验——ROM 启动头已按官方 `.vector` 复刻:入口跳转 + boot option `0xF3F9BDA9` @flash 0x14,见 `ci/check_wch_boot.py`)
  - [x] CH57x: ch572(QingKe **无 A 扩展**,RV32IMC;构建级验证 2026-09-19,真机待验——启动头与 ch583 同布局(官方 `Link.ld` 现场链接实测魔数同样在 flash 0x14);12K SRAM 的栈账已实测缩编:idle/软件定时器任务栈 256 字、示例 3 任务 192 字栈、堆 5.5K,见 `examples/multitask_ch572.rs`)
- CM7F
  - [x] STM32H7B0VBT6
- CM4F
  - [x] STM32F401CCU6
