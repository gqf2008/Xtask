...
作为一名软件领域的工程师，在职业生涯的尽头能有幸接触到一部分硬件产品是我莫大的荣幸。秉承我一贯刨根问底，不搞清楚问题本质不罢休的作风和态度，结合基本的计算机知识加一部分FreeRTOS源码，开始了文档标题中的工作，这项工作带来的价值可以指导未来在嵌入式工程领域软件研发工作（人或事），深入而透彻，也能告诉你怎么通过硬件看清软件运行的本质；同时，也作为在嵌入式领域工程实践方面总结与回顾。

文档很枯燥，大部分内容是教科书上那些陈词滥调的重复引用/解释，可参考的部分应该就剩下那份按部就班实现的工程样板代码。

我尽量用人类容易理解的熟语、类比加上大量的配图等来表达，如果您能坚持阅读下去那么我要恭喜您；如果您能阅读到参考工程的源码部分那么我佩服您；如果您能试着亲手撸一遍那么我要崇拜您；如果您能提出更优雅、更妥当的实现方法或者能指出实现不恰当、错误的地方那么我要仰慕您；如果您能亲手移植一块芯片那么您对操作系统的调度机制应该已经非常通透了，接下来您要思考在多核心环境下如何实现调度，您也可以开始阅读Linux操作系统芯片部分、调度器部分代码了。
...
|名词|说明|
|---|---|
|ISA|（英语：Instruction Set Architecture，缩写为ISA），又称指令集或指令集体系，是计算机体系结构中与程序设计有关的部分，包含了基本数据类型，指令集，寄存器，寻址模式，存储体系，中断，异常处理以及外部I/O。指令集架构包含一系列的opcode即操作码（机器语言），以及由特定处理器执行的基本命令。常用的有x86、ARM、RISC-V、SPARC|
...
- ISA
    ...
    从图上看虽然ISA站在生态链的顶端，具有至高无上的权利和荣誉，但其实也很孤单，有多少曾经的王者纷纷倒下，例如：PA-RISC、PowerPC、SPARC、Alpha、MIPS等，所以有种说法是: ISA不难设计，难的是软硬件生态建设。目前主流的ISA有x86、Arm、Risc-V（开源）。有了ISA，那么接下来就需要配套的软硬件环境了，
...
就是一套满足特定ISA软件开发的工具集。主要包括编译工具、调试工具、硬件虚拟化工具等。常见的开源编译器有gnu-gcc、gnu-g++、clang/llvm、rustc等，商业的有vc、intel-c++、iar、armcc、borland-c++等，调试器有OpenOCD，GDB以及其他跟随 IDE 一起发布的商业调试器，硬件虚拟化工具非大名鼎鼎的QEMU莫属了。
...
链接器输出的就是这种类型的文件，对，它就是个文件，链接器的工作就是把编译器编译出来的很多文件，按照链接脚本的约定/指示组合成这个文件，这份文件有一定的格式要求，而且是可扩展的，以ELF文件为例，它由ELF头和各个段组成，段以.xxx来命名，典型的几个段是.text这个段放代码，将来会被加载器/下载器复制到ROM区，.data这段放已经初始化的数据/常量，它在文件里（ROM）存一份、在RAM里还要占一份运行时的家——上电后启动代码把ROM里的初值搬到RAM（这就是链接脚本里 `.data > REGION_DATA AT > REGION_RODATA` 的含义：VMA 在 RAM、LMA 在 ROM），.bss这个段放未初始化的数据/变量，它在文件里根本不占空间（NOLOAD），上电后启动代码只是把RAM里的这块区域统一清零，所以全局变量的初值是0不是巧合而是被人擦干净了，.rodata这个段放的只读数据，一般整个放在ROM区，.vector这个段放的是向量表，也就是异常/中断号与跳转地址的对应关系表，例如:中断号0->0x80000000，表示0号中断指向0x80000000这个地址，将来CPU如果发过来0号中断的话就先进这个地址拿指令，向量表的首地址通常是硬件复位中断的入口，也就是CPU上电后第一个寻址的地方，也是CPU上电后PC里的值，这个值是ISA约定或者芯片厂商约定的。
...
当CPU上电后在晶振的驱动下，按照PC的指示开始寻址并执行指令，PC的初始值假设被设置为0x80000000，那么CPU先跳到这个地址，发现这个地址里的指令是跳转到Reset 那个中断服务函数的地址，然后就跳过去了开始执行Reset 代码块中的指令了，可能跑着跑着Reset 函数里又跳到了我们常见的main函数里，这个时候CPU的使用权就交到了用户程序了，main函数不返回一直运行下去，运行下去干嘛呢？等个中断信号执行下多任务调度算法就是接下来要干的事情，Reset 函数的生命周期也就到此结束了。

# 多任务调度

## 为什么要有多任务机制

如果没有多任务机制，在MCU中当然也是可以的，而且很多电子产品都没有RTOS把持，因为都是执行一些很简单的任务，所以一般用定时器+中断就可以实现多个代码块之间跳转，操作系统对小应用来说太重了，不合适也没必要。
...
这副图主要用来表达多任务切换机制的核心思想，没有其他用意，如果您读到这段内容时感到对您有所冒犯或者不适，那么请你跳过这段内容直接往下看。
...
- 对话开始了
    ...
    - 一开始老板没有人对话，所以就去打打高尔夫听听小曲（在空闲任务那里放松呢）啥滴，愉悦地放松着自己
    ...
    - 1分钟后（假设TICK周期为1分钟）秘书跑到老板办公室，说:“某某总”您的时间到了，我们某某某委员会有规定，与老板的谈话时间最多1分钟，副总裁不乐意了，说我可是副总裁，好歹也是二把手，我有优先谈话权，而且我还没有和老板谈完，秘书一听对呀，某某委员会是有这么个规定，然后秘书就离开了（高优先级任务抢占）
    ...
    - 后面的内容不再重复，或者您有兴趣的话再编下去，反正日常工作事项就这些且没有经过某某委员会决策绝不能改
...
和我们人类的心脏与大脑一样，CPU也需要外部晶振/振荡电路来带节奏，进而再去驱动我们的代码执行；那么我们的调度器同样也需要有人来带节奏，怎么驱动呢？Timer控制器出场了，Timer控制器里通常配置了一个固定值和一个可以被CPU累加的计数器，上电后CPU已经开始振荡，每振荡一次就会更新Timer控制器里的计数器，当Timer控制器发现计数器的值与那个固定值比较下，如果发现计数器大于等于那个固定值的话就给中断控制器发中断信号，表示定时时间到了，需要我们的调度器赶快去处理，中断控制器发现没有比Timer控制器更高优先级的或者Pending中的中断了，那么就告诉CPU赶紧去SYSTICK_ISR这个函数里执行，其他的任务先放一放；节拍器就干这个活，只要上电它就一直在工作。
...
> **演进注记(2026-08-26):节拍从"恒定"进到"动态"——tickless(第 28 章)。**
...
> 按实测补账+拨回恒定)。执行级验证:qemu_kernel_tests 套件 23/23
> 的第 20–23 项(第 22 项冻眠→UART RX 外部中断唤醒、第 23 项早醒弹
> 墙钟拍账——ch28 章末练习 1 的兑现)。
...
某些场景下，一些任务需要及时获得CPU时间来保证它在特定的时间内处理完任务，所以给任务增加一个优先级标志，调度器就可以按照高优先级先执行低优先级后执行的顺序分配CPU时间了，这就是常说的抢占式调度
...
pub struct Task {
    ...
    //任务栈空间，就是动态申请的一块内存空间，例如：4k字节，32位CPU的话usize=4字节，所以stack的长度为4k/4
...
当任务被创建时，需要对任务做一些初始化工作，关键内容是申请任务栈空间、配置CPU上下文环境并保存在任务栈里，为什么要保存在任务栈里呢，因为当任务首次运行的时候需要从任务栈里恢复出CPU的上下文，特别是PC、SP两个值，这样CPU才能正确的执行任务代码

这块代码因为跟ISA相关所以是需要针对不同芯片实现，阅读这份代码需要你对RISC-V指令集架构中定义的寄存器有一定的了解，下面以GD32VF103芯片为例看看都干了些啥
...
```rust
    ...
    fn save_context(task: &mut Task) {
        unsafe {
             ...
             *  mcause=0xb8000000,Bumblebee ECLIC 模式下进入 ISR 时的中断形式值(bit31=1 是标准语义的中断标志;高位的具体域布局以《Bumblebee内核指令架构手册》为准——标准 RISC-V 语义下 [30:0] 才是编号,这里没有深究,反正真机在跑)
             ...
             *  mstatus=0x00000C80，MPP[12:11]和mpie[7]置位，mpp在机器模式==11，mpie=1当任务恢复后打开全局中断
             *  x31-x11 不写(旧版做法:留着堆上的陈旧数据;现行七个 RISC-V 口已统一改为显式清零+蹦床,见前文注)
             *  x10(a0)=task.args，任务函数参数
             *  x9-x2 同上
             *  x1(ra)=task_exit，任务返回地址
             *  x0 保留，任务栈sp指向这里
             */
...
.macro SAVE_CONTEXT_SOFT_IRQ
  ...
  addi sp, sp, -36 * 4 
  // 把x1写入sp偏移4字节的位置，0(sp)位置保留给x0寄存器，它等于0
  sw x1, 1 * 4(sp) //ra(return address)//返回地址
  // 保留x2,x3,x4栈空间，有点浪费～，但是看起来对齐比较舒服
  sw x5, 5 * 4(sp) //t0(temporary) t[x]临时寄存器(链接寄存器是x1/ra,别被ARM的LR带偏)
  ...
  sw x10, 10 * 4(sp) //a0(function arguments)/rtval(return value)//入参/返回值地址
  sw x11, 11 * 4(sp) //a1(function arguments)/rtval(return value)//入参/返回值地址
  sw x12, 12 * 4(sp) //a2(function arguments)//a[x]入参地址
  sw x13, 13 * 4(sp) //a3(function arguments)
  sw x14, 14 * 4(sp) //a4(function arguments)
  sw x15, 15 * 4(sp) //a5(function arguments)
  sw x16, 16 * 4(sp) //a6(function arguments)
  sw x17, 17 * 4(sp) //a7(function arguments)
...
为了使CPU启动的时候有活干，先给CPU安排一个空的任务，这个任务里啥都不干就是一个死循环，当然也可以加一些CPU使用率统计工作，如果你使用过windows任务管理器，那么你应该可以看到一个idle的进程一直占用着大量的CPU，这个任务也类似。这个任务的特殊性还在于，如果没有其他就绪任务，那么调度器就切到这个任务上，把CPU让给它；这个任务和其他任务在数据结构上没有任何区别。
...
lw t0, 33 * 4(sp)
csrw mepc, t0 // 任务入口的地址在这里（本清单是历史形态：mepc 直指 task.entry。
              // 现行七个 RISC-V 口已统一改为 mepc 指向"首调蹦床"+初始帧
              // 全槽清零——编译器会把入口序言 outlined 成只按 jalr 调用
              // 约定成立的共享 stub，mret 直入会读到未初始化的帧槽而野跳，
              // 蹦床经标准 jalr 调 entry）
lw t0, 34 * 4(sp)
...
*您在阅读汇编代码的时候也不用太抵触，实现大部分业务逻辑的时候我们很少用汇编，因为太难表达了，人类没有经过专门的训练很难像机器一样工作，反人类，这个事情交给编译器去完成就行，那么这里的汇编指令其实没有几条，而且功能也很单一，干的事情也就剩下操作下栈指针偏移量，设置下PC和SP以及CSR寄存器的值，从全局任务指针变量里读出任务的地址（直接读内存地址），汇编代码结束的时候CPU就会从新的PC值开始执行了*
...
所谓二值信号量就是一个值它有两个状态，一个状态标识有信号，一个状态表示没有信号了，当有信号的时候那么任务就可以持有这个信号量，那么另外一个任务就不能获得信号量，当不能获得信号量的时候就把自己给挂起，然后触发一个软中断就可以了，当持有信号量的任务释放信号量的时候通知下刚才那个挂起的任务并把那个任务重新放到就绪队列里就可以了，当下一个tick中断到来时调度器会调度这个就绪任务，那么这个时候这个任务就可以持有信号量了；

二值信号量通常用在两个任务之间协同完成某项工作（谁拿到令牌谁干活，ISR 也可以给令牌）；它和互斥量的关键区别在**所有权**：互斥量谁上锁就只有谁能解锁（信号由持有任务自身获取、释放），而二值信号量任何任务乃至中断服务都可以给信号——所以"任务A上锁、任务B解锁"这种误用在互斥量上根本表达不出来，这也正是用锁保护共享数据的语义所在。（FreeRTOS 还给互斥量配了优先级继承来抑制优先级反转，本参考实现已实现，含 per-task 持锁集合 + 全链重算的完整语义，并更进一步落地了优先级天花板协议 PCP（`Mutex::with_ceiling`，规则 2 的全局登记与天花板阻塞见 `src/sync/lock_core.rs`、书稿第 26 章）——见 `src/sync/lock_core.rs`。）
...
如果信号量只能用来产生信号，那么队列同时可以用来传递数据，多个任务间交换数据是很常见的业务场景。实现的思路就是多种信号量加上数组
...
定时器太有用了，可以用来做计时器，计数器等待，可以按固定周期执行也可以一次性执行，而且干的事情也极其轻量级（不允许在定时器里干特别多的事情，否则会影响其他定时任务的执行），看上去任务也可以干这个事情，但是任务太重了不是吗，我们用一个任务去维护所有的定时任务就可以了
...
> ⚠️ **踩坑记录之二（上面 `do_tick` 和 `Timer` 的清单是旧版，现行代码已修）**：两处跟现行 `src/timer.rs` 不一致，按本文"以参考实现为准"的约定说明如下：
>
> 1. **`do_tick` 只弹了堆顶一个**。上面清单里 `if let Some(timer) = heap.peek() { ... heap.pop() }` 只取一个到期定时器——同一个 tick 到期多个定时器时，其余的都要多等一个 tick 才被逐个弹出，周期定时器会肉眼可见地漂移。**修复后**改成 `while let Some(timer) = heap.peek() { if ticks < timer.next_tick { break; } ... }` 的循环，一次清空所有到期的。
> 2. **`Timer` 句柄没有取消语义**。上面的 `Timer(usize)` 只是个裸值，定时器一旦创建就停不下来。**现行实现**给 `Timer` 加了 `Drop`——句柄离开作用域即从堆里摘除该定时器（周期定时器的闭包也在 Drop 里释放），用法形如 `{ let _t = Timer::period(20, || ...); /* ... */ }`，作用域结束定时器随之取消。这也是第 12 章套件里定时器测试的语义基础。
