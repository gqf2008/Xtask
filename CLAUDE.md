...
Chip features: `gd32vf103`, `stm32f1`, `stm32f4`, `stm32h7`, `cm32m4`, `rp2040`, `ch32v103`/`ch32v203`/`ch32v307` (all build-verified 2026-08-23; real-board verification pending for f4/f1 constants, h7 timeline, cm32m4/rp2040). Non-chip features: `timer` (software timers), `debug_task`, `fs` (fatfs), `net` (smoltcp), `usb`, `ble`, `rtt_log` / `stdout_log`, board BSPs (`longan_nano`, `bluepill`, `greenpill`, `rp_pico`).

## Architecture

Layered, bottom-up:

- **`src/port.rs` — the portability seam.** Trait `Portable` defines the ~10 primitives the kernel needs (memory barrier, critical section `free`, enable/disable interrupt, `start_scheduler`, software `irq`, `systick`, `delay_us`, `save_context`). The type alias `Porting` is bound to exactly one chip implementation via `#[cfg(feature = ...)]`. **All kernel code calls `Porting::...` — never a chip directly.** To port to a new chip: implement `Portable` + provide `memory.x` under `src/chip/<chip>/`, then add the feature wiring in `port.rs`, `chip/mod.rs`, `build.rs`, and `Cargo.toml`.
