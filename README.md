...
- [x] 宿主回归：`cargo test --lib` 默认 features 实测 **88 passed**(2026-09-19 实测;新增 chip::delay 的 6 项纯算术测试后由 82 增加);门禁全特征组合(见 `ci/gate.sh`,含 fs/net/usb/ble)实测 **165 passed**(2026-09-19 实测,rustc nightly-2025-08-08;旧计数 148→159→165 均按实测修正,历史见 issue #11;计数随新增测试浮动,以 `ci/gate.sh` 实测为准)
