...
//!    ——声明责任(天花板必须覆盖所有使用者)在调用方,书稿第 27 章。
//!
...
impl LockCore {
    ...
    /// (书稿第 27 章"声明责任")。0 用 [`new`](Self::new)(纯 PI)。
...
pub(crate) fn acquire(core: &UnsafeCell<LockCore>, reentrant: bool) -> bool {
    ...
    sync::free(|_| unsafe {
        ...
        // 书稿第 27 章)。拦住的任务挂全局天花板阻塞队列,等任何天花板锁
...
/// 天花板(声明责任,书稿第 27 章),规则 1 的"拿锁即升"由此与 PI 的
...
mod tests {
    ...
    /// 失守,引擎按声明行事、不纠正声明——书稿第 27 章"声明责任")。
    #[test]
