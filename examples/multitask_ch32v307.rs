...
// CH32V307(QingKe V4F,RISC-V RV32IMAC softfp)多任务示例——2026-08-23 移植。
// ⚠️ 真机核对点(构建级验证已过,板上行为待验):①HPE 关闭路径
// (intsyscr 0x804 写 0);②mtvec Direct 分发与 mcause=12(SysTick);
// ③SysTick CTLR=0xF 自动重装 + SR.CNTIF 写 0 清零;④SWIE(bit31)触发
// 软中断语义;⑤flash 基址 0x08000000;⑥复位默认 HSI 8MHz(env 按此配)。
...
fn main() -> ! {
    ...
    xtask::init_heap(start_addr, 48 * 1024); // 64K SRAM:静态+栈后保守 48K 堆
