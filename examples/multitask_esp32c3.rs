...
// ESP32-C3(RV32IMC)多任务示例——2026-08-23 移植(PAC 直依赖路线)。
// ⚠️ 真机核对点(构建级验证已过,板上行为待验):①启动方式——riscv-rt
// 直链需 C3 direct boot 模式(flash 头 magic);②SYSTIMER 16MHz 周期
// 模式寄存器序列(target0/comp0_load/int_ena);③中断矩阵 map(5 位值=
// CPU 中断号)与 cpu_int_enable 位图;④from_cpu_0 电平源(ISR 写 0 清);
// ⑤复位默认 CPU 80MHz(env);⑥flash/RAM 基址 0x42000000/0x3FC88000。
// 无 RTT/stdout 配置——日志能力待 BSP 层补(当前纯任务演示)。
...
fn main() -> ! {
    ...
    xtask::init_heap(start_addr, 128 * 1024); // 256K DRAM:静态+栈后保守 128K 堆
