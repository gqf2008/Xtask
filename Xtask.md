...
二值信号量通常用在两个任务之间协同完成某项工作（谁拿到令牌谁干活，ISR 也可以给令牌）；它和互斥量的关键区别在**所有权**：互斥量谁上锁就只有谁能解锁（信号由持有任务自身获取、释放），而二值信号量任何任务乃至中断服务都可以给信号——所以"任务A上锁、任务B解锁"这种误用在互斥量上根本表达不出来，这也正是用锁保护共享数据的语义所在。（FreeRTOS 还给互斥量配了优先级继承来抑制优先级反转，本参考实现已实现，含 per-task 持锁集合 + 全链重算的完整语义——见 `src/sync/lock_core.rs`。）
